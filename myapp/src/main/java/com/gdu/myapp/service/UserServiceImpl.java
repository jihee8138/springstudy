...
public class UserServiceImpl implements UserService {
  ...
  public void signin(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      // DB로 보낼 정보 (email / pw : USER_T , email / IP : ACCESS_HISTORY)
      ...
      if(user != null) {
        ...
        // 회원 정보를 세션(브라우저 닫기 전까지 정보가 유지되는 공간, 기본 3분 정보 유지)에 보관하기 
        // 로그인의 기본 원리 : 세션이라는 저장소 (데이터 바인딩 영역)에 회원 가입한 사람의 정보를 올려두는 것
        request.getSession().setAttribute("user", user);  // 저장한 이름(여기선 "user") 팀원 모두가 공유하고 있어야 한다
        // Sign In 후 페이지 이동
  ...
  public ResponseEntity<Map<String, Object>> checkEmail(Map<String, Object> params) {
    // 탈퇴한 사람이 있는지 점검해야함 (탈퇴했던 이메일은 쓰지 못 함)
    // 사용가능한 이메일이려면 이게 없어야 한다 없어야 사용가능함
    // 둘 다 null 이면 이메일이 없는 것이므로 쓸 수 있는 이메일이다
