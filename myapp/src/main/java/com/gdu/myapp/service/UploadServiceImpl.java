...
public class UploadServiceImpl implements UploadService {
  ...
  public boolean registerUpload(MultipartHttpServletRequest multipartRequest) {
    ...
    int insertAttachCount;  // 첨부가 없어도 files.size()는 1이다.
    if(files.get(0).getSize() == 0) {  // 사이즈 = 0이면 첨부가 안 된 경우
      insertAttachCount = 1;
    } else {
      insertAttachCount = 0;
    }
    ...
    for(MultipartFile multipartFile : files) {
      ...
      if(multipartFile != null && !multipartFile.isEmpty()) {
        ...
        try {
          ...
          // 썸네일 작성
          String contentType = Files.probeContentType(file.toPath());  // file 파일을 path 파일로 바꿔주기 => String이 반환됨
    ...
    return (insertUploadCount == 1) && (insertAttachCount == files.size());  // 첨부 개수와 사이즈가 같아야함
    // 
  ...
  public void loadUploadList(Model model) {
    ...
    // Map에 있는 거 꺼낼 때는 저장된 타입으로 반환이 가능한데 저장이 object로 되어있기 때문에 캐스팅 필요
